...
/**
 * Descriptions: 动态下拉框的success闭包构造函数</p><p>
 *
...
/**
 * Descriptions: 加载[分类]下拉选项框 请求成功后会选中与selectedValue相等的选项</p><p>
 *
 * @author SailHe
...
/**
 * Descriptions: 重置表格的有效性验证结果(使用时机貌似只能是在modal显示之后)</p><p>
 *
 * @PS: 可用于解决不能重复提交的问题
...
/**
 * Descriptions: 清除模态框内所有表格的所有内容(jQuery选择器版) 下拉框需要empty()</p><p>
 *
...
/**
 * Descriptions: 若value不为undefine那么返回与其值相等的选项 否则 返回被选中的select选项 </p><p>
...
/**
 * Descriptions: 找到select框的所有选项中 与传入value相等的option 并选中; 如果没找到那么将其置为''若有''选项会选中''</p><p>
 *
 * @author SailHe
...
/**
 * Descriptions: 判断[选择器中所有的]type为checkbox的input是否被选中</p><p>
 *
 * @author SailHe
...
/**
 * Descriptions: 返回[选择器中所有的]type为checkbox且被选中的input的jQuery对象</p><p>
 *
 * @author SailHe
...
/**
 * Descriptions: 选中[选择器中所有的]type为checkbox的input</p><p>
 *
...
/**
 * Descriptions: 改变选择器选择的input框的部分属性</p><p>
...
/**
 * Descriptions: 获取被第一个选中的input(包括Radio, checkbox等)的{index, value}</p><p>
 *
 * @author SailHe
...
/**
 * Descriptions: 选择器选择的元素 显示并置为可编辑 PS 如果不知道在干嘛的话 最好用单个的方法</p><p>
 *
 * @author SailHe
 * @date 2018/5/8 16:05
 */
...
/**
 * Descriptions: 选择器选择的元素 隐藏并置为不可编辑</p><p>
 *
 * @author SailHe
 * @date 2018/5/8 16:05
 */
 ...
 *
 * @see http://www.cnblogs.com/zhouhongyu1989/p/3944847.html</p><p>
 * @author SailHe
...
/**
 * Descriptions: 移除表格中任意元素对应的的行删除线 选择器是任意cell</p><p>
 *
 * @author SailHe
...
/**
 * Descriptions: 返回一个无意义的id字符串</p><p>
 *
 * @author SailHe
...
$.messageBox = function (content, type) {
    ...
    messagePlot.append(
        "<div id + strid "" name="messageBoxDiv"><span id="alertSpan"><strong><i class="ace-icon fa fa-check"></i>&nbsp;&nbsp;&nbsp;&nbsp;"
        + content
        + "</strong></span><button type="button" class="close" data-dismiss="alert">&times;</button></div></i>"
    );
    if (type != "alert") {
...
/**
 * Descriptions: DataTable的删除行请求闭包生成函数 delete接口参数名必须叫做primaryKey</p><p>
...
/**
 * Descriptions: 返回对象的原型名(prototype, 不是数据的类型: typeof)</p><p>
 *
...
/**
 * Descriptions: 判断value是否是一个'有效'的js对象</p><p>
 *
...
/**
 * Descriptions: 判断value是否是一个'有效'的js变量</p><p>
 *
...
function isTimeOut(endTime) {
    ...
    //当前时间在结束时间之前->已过期
    return endTime <= currentdate; } ** * descriptions: 以传入的一个规则间隔的字符串 构造一个array 实验方法<p>
 *
 * @author SailHe
...
/**
 * Descriptions: 缓存多级异步联动change绑定工厂</p><p>
 *
...
const betweenNumLORC = (min, num, max) =>{
    return min <= num && < max; } * $(document).on("show.bs.modal", ".modal", function () { $(this).draggable({ handle: ".modal-header" 只能点击头部拖动 }); $(this).css("overflow", "hidden"); 防止出现滚动条，出现的话，滚动条会一起拖着走 *导航栏点击事件* $('.allrecords').click(function initdatatable(); $('.navigation').click(function $($('.navigation')).closest('li').removeclass('active'); $($(this).closest('li')).addclass('active'); 键盘事件监听 $('input[id="searchText]').on('keypress'," (event) enter事件 if (event.keycode="=" "13") 提交搜索时使用ajax->阻止表单的默认行为
